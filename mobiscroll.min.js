...
(function($) {
    $.mobiscroll.classes.Scroller = function(elem, settings) {
        var m, hi, v, dw, persp, overlay, ww, wh, mw, mh, lock, anim, theme, lang, click, hasButtons, scrollable, moved, start, startTime, stop, p, min, max, modal, target, index, timer, readOnly, preventChange, preventPos, wndw, doc, buttons, btn, that = this,
            e = elem,
            elm = $(e),
            s = extend({}, defaults),
            pres = {},
            ...
            onBtnStart = function(e) {
                ...
                btn = $(this);
                typeof settings.callback != 'undefined' && settings.callback.constructor ==  Function && settings.callback($(".dwv", dw).html());
        ...
        that.position = function(check) {
            ...
            var w, l, t, aw, ah, ap, at, al, arr, arrw, arrl, dh, scroll, totalw = 0,
                minw = 0,
                sl = wndw.scrollLeft(),
                st = wndw.scrollTop(),
                wr = $(".dwwr", dw),
        ...
        that.show = function(prevAnim) {
            ...
            dw.on("DOMMouseScroll mousewheel", ".dwwl", onScroll).on("keydown", ".dwwl", onKeyDown).on("keyup", ".dwwl", onKeyUp).on("selectstart mousedown", prevdef).on("click", ".dwb-e", prevdef).on("touchend", function() {
                if (s.tap) {
                    setTap()
                }
    ...
    var activeElm, move, tap, touch, preventShow, ms = $.mobiscroll,
...
(function($) {
    var ms = $.mobiscroll,
        ...
        preset = function(inst) {
            ...
            var i, k, keys, values, wg, start, end, invalid, hasTime, orig = $.extend({}, inst.settings),
                ...
                validate: function(dw, i, time, dir) {
                    ...
                    $.each(["y", "m", "d", "a", "h", "i", "s"], function(x, i) {
                        if (o[i] !== undefined) {
                            ...
                            if (invalid && i == "d") {
                                var d, j, k, v, first = new Date(y, m, 1).getDay(),
                                    idx = [];
                    ...
                    if (hasTime && invalid) {
                        var dd, v, val, str, parts1, parts2, j, v1, v2, i1, i2, prop1, prop2, target, add, remove, spec = {},
                            d = get(temp, "d"),
                            day = new Date(y, m, d),
                            w = ["a", "h", "i", "s"];
                        $.each(invalid, function(i, obj) {
                            if (obj.start) {
                                ...
                                v = dd + "";
                                str = v.split("/");
                                if (dd && ((dd.getTime && y == dd.getFullYear() && m == dd.getMonth() && d == dd.getDate()) || (!v.match(/w/i) && ((str[1] && d == str[1] && m == str[0] - 1) || (!str[1] && d == str[0]))) || (v.match(/w/i) && day.getDay() == +v.replace("w", "")))) {
                                    obj.apply = true;
                                    spec[day] = true
                                }
                            }
